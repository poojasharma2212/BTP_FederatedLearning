...
def averageModels(global_model, clients, snr_value, Ps,alpha,K_clients):
    ...
    for k in global_dict.keys():  # key is CNN layer index and value is layer parameters
        ...
        global_dict[k] = torch.stack([client_models[i].state_dict()[k].float() * samples[i]  + torch.randn(client_models[0].state_dict().float().size())*std for i in range(len(client_models))], 0).sum(0)
