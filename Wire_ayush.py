...
global_train, global_test, train_group, test_group = load_dataset(
    args.clients, args.iid)  # load data

# return actual image set for each client
for inx, client in enumerate(clients):
    trainset_ind_list = list(train_group[inx])
    client['trainset'] = getImage(
        global_train, trainset_ind_list, args.local_batches)
    client['testset'] = getImage(global_test, list(
        test_group[inx]), args.local_batches)
    # useful while taking weighted average
    client['samples'] = len(trainset_ind_list) / args.images

# load dataset for global model (to compare accuracies)
transform = transforms.Compose(
    [transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))])
global_test_dataset = datasets.FashionMNIST(
    './', train=True, download=True, transform=transform)
global_test_dataset = torch.utils.data.random_split(
    global_test_dataset, [10000, len(global_test_dataset)-10000])[0]
global_test_loader = DataLoader(
    global_test_dataset, batch_size=args.local_batches, shuffle=True)
...
def ClientUpdate(args, device, client, key_np, key, snr, csi, mu):
    ...
    # ...............................................checking if channel is good enough for transmission by checking BER..................................#
    if(sum(np.bitwise_xor(key, key_np_received))/len(key) == 0 and poptim > 0):
        ...
        for epoch in range(1, args.epochs + 1):
            for batch_idx, (data, target) in enumerate(client['trainset']):
                ...
                # send data to cpu/gpu (data is stored locally)
                ...
                if batch_idx % args.log_interval == 0:
                    ...
                    print('Model {} Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        client['hook'].id,
                        epoch, batch_idx *
                        args.local_batches, len(
                            client['trainset']) * args.local_batches,
    ...
    for fed_round in range(args.rounds):
        ...
        # to check which clients have a good channel, only those will be taken for averaging per round
        client_good_channel = []
        ...
        while(mu <= 1):
            ...
            for jj in csi:
                pn = max(1/mu-1/jj, 0)
                # capacity of a channel (shannon's law)
                g1 += math.log(1+pn*jj)
                pn1 += pn
        ...
        for client in members:
            goodchannel = ClientUpdate(
                args, device, client, key_np, key, snr[index], csi[index], smallmu1)
            if(goodchannel):
